...
void add_menu();
struct Product *add(struct Product *head, char *name,  int inventory,  double price, int sale);
void del_menu();
...
struct Product *edit_sale(struct Product *head, struct Product *temp);
...
struct Product* load(const char* filename)
{
	...
	while (fscanf(fp, "%s %d %lf %d", buf.name, &buf.inventory, &buf.price, &buf.sale) > 0)
	{
		head = add(head, buf.name, buf.inventory, buf.price, buf.sale);
	}
...
void showall()
{
	...
	//加载文件
		...
		{
			printf("Name:%s Inventory:%d Price:%lf Sale:%d\n", p->name, p->inventory, p->price, p->sale);
		}
}
...
void add_menu()
{
	...
	//加载文件
		...
		do
		{
			...
			scanf("%d", &sale);
			//调用插入添加函数
			head = add(head, name, inventory, price, sale);
...
void edit_menu()
{
	...
	//读取文件
		...
		else
		{
			...
			printf("2:> Edit Product Inventory\n");
			printf("3:> Edit Product Price\n");
			printf("4:> Edit Product Sale\n");
...
struct Product* edit_sale(struct Product* head, struct Product* temp)
{
	printf("Please enter Sale to replace %d:\n", temp->sale);
	fflush(stdin);
	scanf("%d", &temp->sale);
	return head;
}
...
void search_menu()
{
	...
	//加载文件
		...
		{
			temp = search(head, name);
			printf("Name:%s Inventory:%d Priec:%lf Sale:%d\n", temp->name, temp->inventory, temp->price, temp->sale);
		}
}
...
void save(struct Product* head, const char* filename)
{
	...
	for (struct Product* cur = head; cur != NULL; cur = cur->next)
	{
		fprintf(fp, "%s %d %lf %d\n", cur->name, cur->inventory, cur->price, cur->sale);
	}
	fclose(fp);
}